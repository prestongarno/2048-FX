...
public class NumberGame implements NumberSlider {
   ...
   public void resizeBoard(int width, int height, int winningValue) {
		...
		cells.stream()
				...
				.forEach(cell -> board[cell.row][cell.column] = cell.value);
		if (this.getNonEmptyTiles().size() == 0) {
   ...
   public Cell placeRandomValue() {
      ...
      ArrayList<Cell> cells = this.getEmptyTiles();
      Cell c = cells.get(current().nextInt(cells.size()));
      c.value = (current().nextInt(2, 6) == 5) ? 4 : 2;
      board[c.row][c.column] = c.value;
		this.score = score + ( c.value * c.value);
		UndoStack.getInstance().notifyMoved(this);
   
      if(cells.size() == 1 && !canMove())
